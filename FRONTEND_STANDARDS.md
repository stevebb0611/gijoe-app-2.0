...
- **Figure input is one flow, one entry point** (locked August 2026, see `OPEN_QUESTIONS_ISSUES_FOUND.md` #25–#26). Every way to add a figure to the collection — the header ＋Add Figure button, a Figure modal's ＋ "add a copy," clicking an unowned catalog figure, an empty Figure Sets slot — opens the *same* Add Figure overlay. No bespoke preview/acquire screen for any of them. Production-variant selection always lives on the DETAILS step, never a separate pick on FIND, and always has a default (the figure's first variant, or — when duplicating an existing copy — that copy's own variant) so advancing past it is never blocked. One picker layout everywhere it appears.
- **Accessory input is one row, everywhere an accessory is owned** (locked August 2026, see `OPEN_QUESTIONS_ISSUES_FOUND.md` #24). Every accessory-owning list — the Figure modal, Add Figure's DETAILS step, the Add Loose Parts flow, Parts Bin's existing entries — renders the same shared row: checkbox/box-grid, owned/required count, then ⚠ damage and ⇄ move/swap-to-bin where applicable. Never a bespoke row component per screen, and never a global "mark as damaged" toggle that opens a second, duplicate list — damage, move, and swap-to-bin are all small popovers anchored to that row's own trigger button.
- **Before deviating from either of the above two rules**, treat it as a design question, not a local styling choice — flag it and get sign-off first, the same standard this section already holds ALL CAPS exceptions to.
