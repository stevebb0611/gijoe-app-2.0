...
### 7.5.2 Release editions — a retail figure re-released through another channel (new — July 2026)

§7.5's "channel differences are just a different Figure row" rule covers a case that's easy to state but, in practice, needed a concrete worked example: **the same character and the same production version, sold once at retail and again later at a convention or mail-in, with a genuinely different accessory set** — e.g. Flint v1 (1985 retail, carded) and its 1992 Convention re-release (bagged, different gear); Quick Kick v1 (1985 retail) and its Convention exclusive (peach-colored skin, different weapons). The owner wants to own and track a copy of *each* edition separately.

**The model needs nothing new** — this is exactly the §7.5 case, applied literally: two `figures` rows, same `code_name`, disjoint `figure_accessories` blueprints, the 700-block row carrying `release_context = 'convention'` (or `'mail_in'`) and `series_id = 15`. No schema change, no override table, no third axis. What actually blocks it in practice is a **seed-pipeline bug**, not a design gap: `server/seed.mjs` groups CSV rows by `(code_name, version, character_key)`, and a retail row + its 700-block sibling routinely share that key (same character, same version number, blank `character_key`) — so the seed silently keeps the lower-id row and drops the other on every from-scratch rebuild. Confirmed for Quick Kick (fixed one-off, see `server/split-quick-kick-convention.mjs`) and Flint (fixed via the generalized tool, `server/split-release-edition.mjs`); ~20 more code names show the identical collision pattern in the source CSV (`OPEN_QUESTIONS.md` Q18) and remain an unaudited backlog.

**Before splitting a colliding pair, verify it's a real second edition, not an accidental CSV duplicate.** Pulling every known colliding pair's raw CSV data (July 2026 audit) found three shapes, not one:
- **Genuine two-edition pairs** — one row cleanly tagged `retail`, the sibling cleanly tagged `convention`/`mail_in` with its own distinct gear (Flint, Quick Kick, Ice Viper, Steeler, Python Officer, Python Tele-Viper, Motor Viper·A). These are real splits.
- **Likely accidental duplicates** — *both* colliding rows tagged the same non-retail context with near-identical data, sometimes with a corrupted field between them (a flipped `faction_id`, a swapped detail). These aren't two collectible editions — they're the same release entered into the CSV twice. Splitting one of these would manufacture a second ownable catalog entry for something that was only ever sold once. Undercover Scarlett is this shape: convention-exclusive, but only *one* real edition — the fix there is making sure the seed keeps a correct single row, not splitting it into two.
- **Ambiguous** — differing free-text (e.g. two different named conventions) that could be two real events or a mislabel. Needs a real reference before either treatment.

`server/split-release-edition.mjs` is the reusable tool for the first case: a small array of confirmed split specs (retail figure_id, new 700-block figure_id, the fields that differ, the accessory codes that belong on the new row's own blueprint) — applying a newly-confirmed split means appending an entry, not writing a new script. Don't add an entry from CSV data alone; the CSV's own accessory linkage for a dropped 700-block row can itself be wrong (Flint's F701 pointed at Ambush's gear in the source data, not Flint's) — confirm against a real reference or, ideally, a physically-owned copy before writing the accessory list.

**`FIGURE_SPLITS.md` is the per-figure operational log** for this mechanism — the same role `ACCESSORY_GROUPS.md` plays for `group_id`/`match_key`/`variant_id`. Every figure actually split (or a colliding pair resolved some other way, e.g. Jinx's miscategorization fix) gets an entry there. It also draws the line against `ACCESSORY_GROUPS.md`'s separate `release_context`-on-one-row mechanism (§7.5's "lone accessory that only came via a channel" case) — several collision-list figures already got that lighter treatment instead of a split; see that doc for which.
