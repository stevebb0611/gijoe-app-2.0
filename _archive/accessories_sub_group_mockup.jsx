// accessories_sub_group_mockup.jsx
// =============================================================================
// ARCHIVED 2026-07-03 — superseded. GroupCard's group_id/release_context
// model and the AccSwatch/colorFor colour designations described below are
// now live in production: web/src/accessory-groups.jsx (VariantGroup,
// MatchedGroup, ContextGroup), web/src/acc-colors.jsx (colorFor/AccSwatch),
// web/src/app-detail.jsx + web/src/app-add-figure.jsx (row rendering). Kept
// here for historical rationale only — do not treat as current spec; do not
// import from a live component.
// =============================================================================
//
// LOCKED REFERENCE (June 2026) — accessory completeness model: variant
// grouping (group_id) + release_context.
//
// Source: design_handoff_inventory/subgroup-wire-v2.jsx (verbatim, "Option C"
// wireframe). Renamed/annotated here so it can be diffed against and merged
// into the production component tree in VS Code. Paired reference HTML:
// "GI Joe Tracker - Accessory Sub-Groups v2.html". Full spec prose lives in
// design_handoff_inventory/PARTS_BIN.md ("Accessory completeness model").
//
// Depends on globals from acc-colors.jsx: window.colorFor(name), <AccSwatch>.
// Depends on <DesignCanvas>/<DCSection>/<DCArtboard> only in the demo App at
// the bottom of the reference HTML — NOT part of the product component tree,
// safe to drop when merging.
//
// -----------------------------------------------------------------------------
// THE DATA MODEL (faithful to the live `figures_accessories` join — no schema
// invented for the mockup):
//
//   figure.acc = [
//     { id, name, color, qty?, group_id?, ctx? }
//     …
//   ]
//
// A figure owns a FLAT list of accessory rows. Two fields — and ONLY these
// two — restructure that flat list into anything other than a plain
// checklist. They are independent axes; either, both, or neither can apply
// to a given row.
//
//   • qty (quantity_required)  — defaults to 1. >1 renders as N tick boxes
//     instead of one checkbox (e.g. Scrap-Iron's Missiles ×2).
//   • group_id                 — clusters 2+ accessories as INTERCHANGEABLE
//     VARIANTS. Owning ANY ONE member satisfies the whole group.
//   • ctx (release_context)    — where the piece came from. Default/absent
//     = "retail" (counts toward completion). Any other value — convention,
//     bonus, mail-in, exclusive — is TRACKED but EXCLUDED from completion.
//
// -----------------------------------------------------------------------------
// 1. group_id → "own any one" variant box
//    Two+ rows sharing a group_id render as ONE bordered box: a centered
//    shared label + centered option buttons with "or" between them.
//    - No per-item "owned" status line, no "own either" instruction copy —
//      dropped in review. The filled checkbox on the selected option is the
//      only state shown.
//    - Generalises: ANY set of rows under one group_id renders this same
//      box. Nothing is special-cased per figure.
//    - Examples: Scrap-Iron's Remote Activator thin/thick handle = group_id
//      8401. A.V.A.C.'s Parachute pack soft/hard plastic = group_id 8601
//      (his ENTIRE kit is this one group — the card is just the box).
//
// 2. No group_id → a plain row (qty-aware)
//    Every ungrouped accessory is its own row: single checkbox at qty 1, or
//    N tick boxes at qty > 1. Multi-piece assemblies are NOT a special
//    "assembly" construct — e.g. Scrap-Iron's Missile System is six plain
//    accessory rows (Legs ×2, Missiles ×2 among them) because the data
//    carries no grouping binding them together. (An "assembly" box was
//    tried and reverted — "too extra".)
//
// 3. release_context → an independent completion axis
//    Orthogonal to group_id. retail (default) counts; convention / bonus /
//    mail-in / exclusive are tracked but never block Complete. Non-retail
//    rows pull into their OWN per-context box (header = context name, e.g.
//    "Convention"), grouped in first-seen order.
//    - Demo: Cobra trooper — the retail Dragunov rifle is the only thing
//      required; the four convention pieces (M-16 MG, Bipod, two Bazooka
//      handles) sit in a "Convention" box and stay optional. Card reads
//      Complete even with convention pieces unowned.
//    - The two axes COMPOSE: a context box could itself contain a group_id
//      variant pair (e.g. the convention Bazooka thin/thick handles) once
//      those ids are assigned in the data.
//
// 4. Completeness rule (LOCKED)
//    Complete when: every RETAIL plain row is owned at its qty, AND every
//    RETAIL group_id has >=1 member owned. Non-retail contexts are ignored
//    entirely. Footer is binary — "Complete" / "Incomplete", sentence case,
//    no reason string.
//
// 5. Visual rules carried from review
//    - Checkboxes fill ONE neutral confirm color (var(--ok), green) — they
//      do NOT echo the accessory's own color; that read as overstimulating.
//      The accessory's color swatch lives only on the row label (supports
//      two-tone "x/y" swatches, e.g. "silver/purple").
//    - Alt-names (e.g. "Scrap-Iron" / "Scrap Iron") ride in the card header
//      as an A.K.A. line, sourced from alt_name.
//
// Open / deferred (per PARTS_BIN.md):
//    - A better in-card notation for release_context — an explicit
//      "not counted" tag was tried and dropped; treatment still TBD.
//    - Whether owning BOTH members of a group_id should flag a "variant
//      spare" — currently silent.
//    - group_id / release_context aren't in the current join export; this
//      mockup hardcodes the owner-supplied ids (8401, 8601) and contexts
//      for illustration only.
//
// -----------------------------------------------------------------------------
// COMPONENT MAP IN THIS FILE
//   OptionPills2 / VariantPill / PlainRow / Foot / CardHead
//     — the earlier "matched colour SET" mechanic (Firefly / Rock 'N Roll):
//       a figure has qty-aware "slots" (e.g. Submachine Gun, Missile ×2)
//       and each slot can be filled in one of several named colour
//       "versions." Complete = all base singles + one version where every
//       slot hits its required qty in that shade. Kept in this file as
//       prior art; NOT the same mechanic as group_id (versions apply
//       per-slot across ALL slots at once; group_id is per-row, "any one").
//   GroupCard / clusterAcc / groupLabel / optLabel / QtyBoxes
//     — the CURRENT/LOCKED mechanic described in the numbered sections
//       above: flat acc[] -> group_id boxes + ctx boxes + plain rows.
//       This is the component to carry into production.
//
// Sample figures included: FIREFLY, ROCK ('matched set' demo) · SCRAP,
// AVAC, COBRA_TROOPER ('group_id + release_context' demo — COBRA_TROOPER is
// the purple "convention" tag example).
// =============================================================================
...
// but sit OUTSIDE the formula, shown in their own context box (the purple
// "Convention" tag in the mockup screenshots). `ctx` mirrors the join's
// release_context column (retail | convention | bonus | mail-in |
...
// (Matched-set mechanic — see file header. Prior art, not the group_id path.)
// =========================================================================
...
// =========================================================================
// GROUP CARD — THE LOCKED COMPONENT (June 2026). Flat accessory list;
// group_id is the ONLY thing that boxes into a variant group; release_context
// (ctx) is the ONLY thing that splits rows into their own optional box.
// Singles render as plain rows (qty-aware). No prose annotations live in the
// card — those are handoff notes and sit beside the card on the canvas, not
// in product UI.
//
// Completeness = every RETAIL plain row at its qty + every RETAIL group_id
// with >=1 member owned. Non-retail (ctx) rows are display-only, never
// gate the footer.
// =========================================================================
