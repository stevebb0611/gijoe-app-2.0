// ⚠️ frozen prototype mirror — live code is web/src/app-add-figure.jsx, do not edit here.
// app-add-figure.jsx — WORKING Add Figure flow (overlay). Searches the real
// catalog (window.JOE_CATALOG), and on finalize writes a real owned instance to
// JoeStore. FIND → DETAILS → CONDITION → FINALIZE. Damage map from damage-map.jsx.
const { DamageMap, GradeBadge, physicalGrade, paintGrade, dmEmpty } = window;
const AF_CATALOG = window.JOE_CATALOG || [];
...
function AddFigureOverlay({ onClose, presetCatalogId = null, presetVariant = null }) {
  ...
  const ownedHere = fig ? window.JoeData.ownedCount(fig.id, variantKey) : 0;
          ...
          {step === 0 && (
            <div className="af-find">
              ...
              <div className="af-results">
                {results.map(f => {
                  const own = window.JoeData.ownedCount(f.id);
                    ...
                    <div className="af-varpick">
                      ...
                      {f.variants.map(v => (
                        <button key={v.letter} className={"af-var" + (selVar === v.letter ? " is-sel" : "")} onClick={() => setSelVar(v.letter)}>
                          ...
                          <span className="af-var__own">{window.JoeData.ownedCount(f.id, v.letter) === 0 ? "not owned" : "×" + window.JoeData.ownedCount(f.id, v.letter)}</span>
                        </button>
                      ))}
                    </div>
                  )}
...
Object.assign(window, { AddFigureOverlay });
