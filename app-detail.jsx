...
// Loaded after damage-map.jsx, before app-inventory.jsx.

const INV_CAT = window.JOE_CATALOG || [];
const INV_ERAS = window.JOE_ERAS || {};
...
const { physicalGrade, paintGrade, dmEmpty } = window;
...
// ---------------------------------------------------------------------------
function fvm(cf) {
  const sum = window.JoeData.figureSummary(cf.id);
...
function figState(fig) {
  const sum = fig._sum || window.JoeData.figureSummary(fig.id);
...
function applyRebalance(catalogId, mode) {
  ...
  st._copies.forEach((c, i) => {
    ...
    window.JoeStore.updateInstance(c.id, { acc: accMap });
  });
}
...
function yearParts(yearNum) {
  ...
  roster.forEach(cf => { const s = window.JoeData.figureSummary(cf.id); if (s && s.owned > 0) { owned++; if (s.whole > 0) completeNow++; } });
...
function InvDetailModal({ catalogId, instId, onClose, onAddInstance }) {
  ...
  const raw = cur ? (window.JoeStore.get().instances.find(i => i.id === cur.id) || {}) : {};
  const moc = !!raw.moc;
  ...
  const setUnit = (name, n) => window.JoeStore.setAcc(cur.id, name, n);
  const setMoc = (v) => window.JoeStore.updateInstance(cur.id, { moc: v });
  const setCard = (patch) => window.JoeStore.updateInstance(cur.id, { filecard: { ...filecard, ...patch } });
  const setNotes = (v) => window.JoeStore.updateInstance(cur.id, { notes: v });
  const setLoc = (v) => window.JoeStore.updateInstance(cur.id, { loc: v });
  const setMarks = (val) => {
    const pg = physicalGrade(val), pt = paintGrade(val);
    window.JoeStore.updateInstance(cur.id, { marks: val, phys: pg.zones ? pg.grade : null, paint: pt.zones ? pt.grade : null });
  };
  const removeCopy = () => {
    ...
    if (kept.length) {
      ...
      if (keep) window.JoeStore.depositParts(catalogId, kept.map(([accessory, qty]) => ({ accessory, qty })));
    }
    window.JoeStore.removeInstance(cur.id);
